            ...
            if (sexo<>'') then begin
              if (nacionalidad<>'') then begin
                ...
                try
                    ...
                    FDQuery2.Open;
                    FDQuery2.Insert;
                    FDQuery2.Params.ParamByName('ID_Noticia').Value := array_idurls[noticia_index];
      ...
      if CCAA<>'' then begin
            if (provincia<>'') then begin
             ...
             FDConnection2.Connected:=true;
              ...
              FDQuery2.Params.ParamByName('ID_Noticia').AsInteger := array_idurls[noticia_index];
              FDQuery2.Params.ParamByName('Fecha').AsString := datetostr(fecha);
              FDQuery2.Params.ParamByName('ID_CCAA').AsInteger := combo_ccaa.ItemIndex +1;
              FDQuery2.Params.ParamByName('ID_Provincia').AsInteger := strtoint(array_Prov[combo_provincia.ItemIndex]);
              FDQuery2.Params.ParamByName('ID_Municipio').AsInteger := strtoint(array_Muni[combo_Municipio.ItemIndex]);
              FDQuery2.ExecSQL;
...
showmessage(array_Prov[combo_provincia.ItemIndex]);
 ...
 if(combo_provincia.ItemIndex>=0) then begin
  ...
  FDQuery2.SQL.Text := 'Select nombre, ID_Mun from Municipios where ID_CCAA like '+ inttostr(combo_ccaa.ItemIndex+1)+' and ID_Prov like '+array_Prov[combo_provincia.ItemIndex] ;
   FDQuery2.Active := True;
   combo_Municipio.Clear;
   SetLength(array_Muni, 0);
   array_Muni:=nil;
   While not FDQuery2.Eof do
   begin

      SetLength(array_Muni, Length(array_Muni)+1);
      array_Muni[High(array_Muni)]:=FDQuery2.Fields[1].AsString;
...
begin
  ...
  begin
   ...
   FDQuery3.Active := False;
    ...
    FDQuery4.SQL.Text := 'Select * from Noticias';
    FDQuery4.Active := True;
    while not  FDQuery4.Eof do
    begin
      SetLength(array_idurls, Length(array_idurls)+1);
      array_idurls[High(array_idurls)]:=FDQuery4.Fields[0].AsInteger;
      SetLength(array_urls, Length(array_urls)+1);
      array_urls[High(array_urls)]:=FDQuery4.Fields[1].AsString;
      FDQuery4.Next;
    end;
    FDQuery4.Active := False;
    finally
